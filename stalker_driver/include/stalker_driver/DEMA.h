...
class EMAFilter {
 ...
 public:
  EMAFilter(float alpha, float initialOutput = 0) : _output(initialOutput) { setAlpha(alpha); }
...
class DEMAFilter {
 ...
 public:
  DEMAFilter(float alpha, float initialOutput = 0)
    : _filter(alpha, initialOutput), _recursiveFilter(alpha, initialOutput) {
    setAlpha(alpha);
